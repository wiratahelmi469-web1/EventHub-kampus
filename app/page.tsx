...
export default function Home() {
  ...
  const [currentUserRole, setCurrentUserRole] = useState<'guest' | 'mahasiswa' | 'panitia' | 'po' | 'staf'>('po');
  const [activeTab, setActiveTab] = useState<'beranda' | 'jelajahi' | 'tugas' | 'koordinasi' | 'event_saya' | 'notif' | 'profil'>('beranda');
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] font-sans antialiased text-slate-800 flex flex-col relative pb-16 lg:pb-0">
      ...
      <div className="bg-slate-900 text-slate-100 px-4 py-2 flex flex-col lg:flex-row items-center justify-between gap-2 border-b border-slate-800 text-xs z-40">
        ...
        {/* DEMO CONTROLS: Switch Role Point of View instantly */}
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-slate-400 whitespace-nowrap"><Shield className="w-3.5 h-3.5 inline mr-1 text-sky-400" /> Switch POV Role:</span>
          <div className="flex bg-slate-850 p-0.5 rounded-lg border border-slate-700">
            {(['guest', 'mahasiswa', 'panitia', 'po', 'staf'] as const).map((role) => (
              <button
                key={role}
                onClick={() => handleRoleChange(role)}
                className={`px-2.5 py-1 rounded-md font-medium capitalize transition-all text-[11px] ${
          ...
          <button 
            ...
            className="bg-slate-800 hover:bg-slate-700 px-2.5 py-1 rounded-md text-[11px] font-semibold text-slate-300 flex items-center gap-1 border border-slate-700"
            title="Reset to Splash"
          >
            <Sparkles className="w-3.5 h-3.5 text-[#FFC107]" /> Splash
          </button>
        </div>
      </div>
        ...
        <div className="flex-grow flex flex-col">
          ...
          <header className="bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-8 py-3.5 shadow-sm sticky top-0 z-30">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Notification bell triggering tab directly */}
              <button 
              ...
              <div className="flex items-center gap-2 border-l pl-4">
                <div className="text-right hidden sm:block">
                  <p className="text-xs font-bold text-slate-900">
                    {currentUserRole === 'po' ? 'Rian Prasetya' :
                     currentUserRole === 'staf' ? 'Tri Wahyuni (Kemahasiswaan)' :
                     currentUserRole === 'panitia' ? 'Dian Permata (Korlap)' :
                     currentUserRole === 'mahasiswa' ? 'Helmi Wirata' : 'Tamu Unregistered'}
                  </p>
                  <p className="text-[10px] text-slate-400 uppercase tracking-tight">
                    {currentUserRole === 'po' ? 'Project Officer' :
                     currentUserRole === 'staf' ? 'Fakultas SI' :
                     currentUserRole === 'panitia' ? 'Divisi Acara' :
                     currentUserRole === 'mahasiswa' ? 'NIM 1202220136' : 'Guest Mode'}
                  </p>
                </div>
                <div onClick={() => setActiveTab('profil')} className="w-9 h-9 rounded-full overflow-hidden cursor-pointer bg-[#1976D2]/10 border-2 border-[#1976D2] flex items-center justify-center text-[#1976D2] font-extrabold text-sm shadow-inner transition-all hover:scale-105">
                  {currentUserRole === 'po' ? 'RP' :
                   currentUserRole === 'staf' ? 'TW' :
                   currentUserRole === 'panitia' ? 'DP' :
                   currentUserRole === 'mahasiswa' ? 'HW' : 'G'}
                </div>
              </div>
            </div>
          </header>
